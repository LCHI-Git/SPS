...
<div style="border: 1px solid #d4c8cc; padding: 15px; width: 100%; height: 450px">
	<div style="height: 100%; width: 40%; position: relative; padding: 15px;">
		<div class="imgDiv" style="width: 100%;">
			<img src= "${product_imgPath}${product_category}.jpg">
		</div>
	</div>
	<div class="contentsDiv" style="height: 100%; width: 60%; position: relative;">
		<div class="nameDiv">${productList.product_name}</div>
		<div class="priceDiv">
...
<!-- 상품 사진 상세 이미지 -->
상품 인덱스 : ${product_idx}<br/>
상세 이미지 개수 : ${product_imgPathStock}<br/>
이미지 경로 : ${product_imgPath}<br/>
이미지 카테고리 : ${product_category}<br/>
이미지 경로(카테고리가 10이상일 때) : ${product_subImgPath}<br/>
<div>
	<c:if test="${product_imgPathStock > 0}">
		<c:forEach var="i" begin="1" end="${product_imgPathStock}" step="1">
			<div class="infoImg" style="margin: 4%;">
<img alt="상품" src="${product_imgPath}${product_category}/${product_category}_/${product_category}_1(1).jpg"/>
			</div>
		</c:forEach>
	</c:if>
</div>
