...
//==============	핸드폰 결제에 필요한 설정 값	==========
	...
	@Autowired
	public SqlSession shopSqlSession;	// sqlSession 변수 >> Autowired로 자동 주입 servlet-context.xml 에 빈 선언해놓음
// ================================================
...
//		MybatisSampleDAO mapper = sqlSession.getMapper(MybatisSampleDAO.class);
	//	
//		ProductVO vo = mapper.getProduct(pNum);
	//	
...
//			주의사항은 폼태그 내의 태그들 name이 사용하려는 vo의 변수와 맞아야함 꼭 기억하세요
			...
			System.out.println("sadadsashssasadsajkdhsakjdhadhsakjdhsajkhdaskjdhaskdhakjdhakjdha");
//			만약에 처음 문자전송 시도이면 아래 코드 들어감
			if(a==1 && !(clientVO.getClient_phoneNumber().length()==0)) {
...
//	결제 완료 페이지
	...
	public String pay3(HttpServletRequest request, Model model) {
